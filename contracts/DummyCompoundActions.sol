...
error DelegateCallError(bytes data);

contract DummyCompoundAction {
    ...
    function supplyERC20Collateral(
        ...
        // delegatecall ctoken.mint() so msg.sender continues to be userwallet contract.
        (bool success, bytes memory returndata) = address(cToken).delegatecall(
            abi.encodeWithSelector(
                cToken.mint.selector, 
                _underlyingToSupplyAsCollateral
            ));
        // FIXME = success is returned false with returndata = 0x.
        if (!success) {
            revert DelegateCallError(returndata);
        }
        uint256 error = abi.decode(returndata, (uint256));
        console.log("error while minting %d", error);
        ...
        console.log("Minted ctoken this sc %d", cToken.balanceOf(address(this)));
        console.log("Minted ctoken msg.sender %d", cToken.balanceOf(msg.sender));
